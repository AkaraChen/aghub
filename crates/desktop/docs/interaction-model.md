...
- **区段顺序**:自定义分组(一等,粗体卡片)在最上;其下是一个**统一的 loose 列表**——来源(source)聚合与散落的未分组项**同一层级**混排,每个条目按 收藏优先 → 名字(来源按末段名)排序。来源不是独立区段,而是一行「带折叠箭头的普通 skill 行」:同样的字重/大小,点击选中全组、可拖拽、右键 items 菜单,只是多了展开/收起。来源簇内任一 skill 被收藏,整簇浮到 loose 列表顶部(高于未收藏的簇和散项)。
- 跨 section 方向键焦点桥:不做(v3 候选)。
- source 簇不可作为落点(它不是用户分组);拖到 loose 区域 = 移出到未分组。
