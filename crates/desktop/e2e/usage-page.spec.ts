...
/**
 * Dedicated /usage page smoke: renders one card per agent the summary reports,
 * with quota bars for the quota agents (claude/codex), a totals breakdown that
 * drops zero-valued rows, and the ccusage status chip. Boots through the shared
 * Tauri/API mocks.
 */

const agent = (id: string, totals: Record<string, number | null>) => ({
	agent: id,
	days: [],
	totals,
});
...
test("usage page lists agents with quota bars and totals", async ({ page }) => {
	...
	// ...and its cost + a non-zero breakdown row.
