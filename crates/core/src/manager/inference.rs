...
impl ConfigManager {
	pub fn import_credientials(&self) -> Result<Vec<Credential>> {
		self.ensure_global_scope("import credentials")?;
		self.adapter.import_credientials(
	...
	fn supports_credential_type(&self, cred_type: CredentialType) -> bool {
		match cred_type {
			CredentialType::Openai => {
				self.descriptor().capabilities.inference.openai
			}
			CredentialType::Anthropic => {
				self.descriptor().capabilities.inference.anthropic
			}
		}
	}
...
pub fn transfer_opencode_providers_to_codex() -> Result<ProviderTransferResult>
{
	...
	let credentials = source.import_credientials()?;
	target.import_inference_providers_to_codex(&credentials)
}

fn load_codex_config(path: &PathBuf) -> Result<CodexConfigFile> {
