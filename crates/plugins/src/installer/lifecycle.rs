...
use super::version::{compare_versions, is_semantic_version};
...
impl PluginInstaller {
	...
	pub(super) async fn cleanup_installation_path_if_unused(
		&self,
		install_path: &Path,
	) -> Result<()> {
	...
	fn get_registry(&self, source: &str) -> Result<Box<dyn PluginRegistry>> {
		match source {
			...
			_ => match classify_registry_source(source) {
				...
				RegistrySource::Local { path } => {
					Ok(Box::new(super::registry::LocalRegistry::new(path)))
				}
