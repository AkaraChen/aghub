...
fn data_dir() -> Option<PathBuf> {
	home_dir().map(|home| home.join(".kode"))
}
...
fn global_skills_paths() -> Vec<PathBuf> {
	...
	vec![home.join(".kode/skills"), home.join(".claude/skills")]
}

fn project_skills_paths(root: &Path) -> Vec<PathBuf> {
	vec![root.join(".kode/skills"), root.join(".claude/skills")]
}
...
pub const DESCRIPTOR: AgentDescriptor = AgentDescriptor {
	...
	project_markers: &[".kode"],
	skills_cli_name: Some("kode"),
	rule_paths: None,
};
