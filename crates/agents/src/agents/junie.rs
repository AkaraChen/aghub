...
fn data_dir() -> Option<PathBuf> {
	home_dir().map(|home| home.join(".junie"))
}
...
fn global_skills_paths() -> Vec<PathBuf> {
	...
	vec![home.join(".junie/skills"), home.join(".agents/skills")]
}

fn project_skills_paths(root: &Path) -> Vec<PathBuf> {
	vec![root.join(".junie/skills"), root.join(".agents/skills")]
}
...
pub const DESCRIPTOR: AgentDescriptor = AgentDescriptor {
	...
	display_name: "Junie",
	surfaces: &[
		AgentSurface::cli("cli", &["junie"], &[data_dir], &["--version"]),
