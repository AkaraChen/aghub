...
fn data_dir() -> Option<PathBuf> {
	home_dir().map(|home| home.join(".neovate"))
}
...
fn global_skills_paths() -> Vec<PathBuf> {
	...
	vec![home.join(".neovate/skills"), home.join(".claude/skills")]
}

fn project_skills_paths(root: &Path) -> Vec<PathBuf> {
	vec![root.join(".neovate/skills"), root.join(".claude/skills")]
}
