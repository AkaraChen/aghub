...
pub fn execute(manager: &mut ConfigManager, resource: ResourceType, name: String) -> Result<()> {
    match resource {
        ResourceType::Skills => {
            ...
            println!("{}", serde_json::to_string_pretty(&json!({"enabled": false, "name": name, "type": "skill" }))?);
        }
        ResourceType::Mcps => {
            eprintln_verbose!("Disabling MCP server: {}", name);
            manager.disable_mcp(&name)?;
            eprintln_verbose!("MCP server disabled successfully");
            println!("{}", serde_json::to_string_pretty(&json!({"enabled": false, "name": name, "type": "mcp" }))?);
        }
        ResourceType::SubAgents => {
            ...
            println!("{}", serde_json::to_string_pretty(&json!({"enabled": false, "name": name, "type": "sub-agent" }))?);
        }
    }
