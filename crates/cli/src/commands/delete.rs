...
pub fn execute(manager: &mut ConfigManager, resource: ResourceType, name: String) -> Result<()> {
    match resource {
        ResourceType::Skills => {
            ...
            println!("{}", serde_json::to_string_pretty(&json!({"deleted": true, "name": name, "type": "skill" }))?);
        }
        ResourceType::Mcps => {
            eprintln_verbose!("Deleting MCP server: {}", name);
            manager.remove_mcp(&name)?;
            eprintln_verbose!("MCP server deleted successfully");
            println!("{}", serde_json::to_string_pretty(&json!({"deleted": true, "name": name, "type": "mcp" }))?);
        }
        ResourceType::SubAgents => {
            ...
            println!("{}", serde_json::to_string_pretty(&json!({"deleted": true, "name": name, "type": "sub-agent" }))?);
        }
    }
