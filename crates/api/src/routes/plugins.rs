use crate::dto::plugin::{
    ...
    HookActionResponse, HookEventResponse, HookMatcherResponse,
    HooksManifestResponse, InstallPluginRequest, InstallPluginResponse,
    McpConfigResponse, McpServerResponse, PluginAuthorResponse,
    PluginDetailResponse, PluginListResponse, PluginManifestResponse,
    PluginResponse, UninstallPluginRequest, UninstallPluginResponse,
...
pub fn get_plugin_detail(plugin_id: String) -> ApiResult<PluginDetailResponse> {
    ...
    let plugin = manager.get_plugin(&id)
        .ok_or_else(|| crate::error::ApiError::not_found(format!("Plugin '{}' not found", plugin_id)))?;

    // Read manifest
    let manifest = plugin.read_manifest()
        .map_err(|e| crate::error::ApiError::internal(format!("Failed to read manifest: {e}")))?;
    ...
    // Read hooks
    let hooks = plugin.read_hooks()
        .map_err(|e| crate::error::ApiError::internal(format!("Failed to read hooks: {e}")))?;

    let hooks_response = hooks.map(|h| {
        let events: Vec<HookEventResponse> = h.hooks.into_iter()
            .map(|(event, matchers)| {
                HookEventResponse {
                    event,
                    matchers: matchers.into_iter()
                        .map(|m| HookMatcherResponse {
                            matcher: m.matcher,
                            hooks: m.hooks.into_iter()
    ...
    // Read MCP config
    let mcp_config = plugin.read_mcp_config()
        .map_err(|e| crate::error::ApiError::internal(format!("Failed to read MCP config: {e}")))?;

    let mcp_response = mcp_config.map(|c| {
        let servers: Vec<McpServerResponse> = c.mcp_servers.into_iter()
...
pub async fn check_plugin_update(
    ...
    let id = PluginId::parse(&req.plugin_id)
        .map_err(|e| crate::error::ApiError::bad_request(format!("Invalid plugin ID: {e}")))?;
    ...
    let plugin = manager.get_plugin(&id)
        .ok_or_else(|| crate::error::ApiError::not_found(format!("Plugin '{}' not found", req.plugin_id)))?;
