...
"#,
    ...
    let props = read_properties(&skill_dir).unwrap();
    assert_eq!(props.metadata.get("author"), Some(&"Test Author".to_string()));
...
fn test_find_skill_md_accepts_lowercase() {
    ...
    assert!(result.unwrap().file_name().unwrap().to_string_lossy().to_lowercase() == "skill.md");
}
...
"#,
    ...
    let props = read_properties(&skill_dir).unwrap();
    assert_eq!(props.allowed_tools, Some("Bash(jq:*) Bash(git:*)".to_string()));
